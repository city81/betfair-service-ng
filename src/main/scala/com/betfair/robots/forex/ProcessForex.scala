...
object ProcessForex extends App {
  ...
  def process() = {
    ...
    var winnersPlusLosersThreshold: Int = 10
    var profitThreshold: Double = 10000.0

    val forexDataList = new ListBuffer[ForexDetail]

//    val fileLines = io.Source.fromFile("/Users/geraint/Documents/FOREX/Forex.csv").getLines.drop(1).toList
//val fileLines = io.Source.fromFile("/Users/geraint/Documents/FOREX/EURGBP_Stoc.csv").getLines.drop(1).toList
    val fileLines = io.Source.fromFile("/Users/geraint/Documents/FOREX/EURUSD_Stoc.csv").getLines.drop(1).toList
...
//    val markets = Vector("EURUSD", "EURGBP", "GBPUSD")
      ...
      for (lossLimit <- 15 until 30 by 5) {
        for (profitLimit <- 10 until 30 by 5) {
          ...
          if ((profit > profitThreshold) && ((winners + losers) >= winnersPlusLosersThreshold)) {
            ...
            + ", potential profit = "
              + ((((winners / (winners + losers)) * not_sure) * profitLimit)
              - (((losers / (winners + losers)) * not_sure) * lossLimit) + profit)
            )
        ...
        for (profitLimit <- 10 until 30 by 5) {
          ...
          if ((profit > profitThreshold) && ((winners + losers) >= winnersPlusLosersThreshold)) {
            println("market = " + market + ", loss limit = " + lossLimit + ", profit limit = " + profitLimit + ", profit = " + profit +
              ", win = " + winners + ", loss = " + losers + ", not sure = " + not_sure
              + ", potential profit = "
              + ((((winners.toDouble / (winners + losers)) * not_sure) * profitLimit)
              - (((losers.toDouble / (winners + losers)) * not_sure) * lossLimit) + profit)
            )
            if (profitMap.contains(market)) {
              if (profit > profitMap.get(market).get._1) {
        ...
        for (profitLimit <- 10 until 30 by 5) {
          ...
          if ((profit > profitThreshold) && ((winners + losers) >= winnersPlusLosersThreshold)) {
            println("market = " + market + ", loss limit = " + lossLimit + ", profit limit = " + profitLimit + ", profit = " + profit +
              ...
              + ((((winners.toDouble / (winners + losers)) * not_sure) * profitLimit)
              - (((losers.toDouble / (winners + losers)) * not_sure) * lossLimit) + profit)
              + ", not_sure % = " + (not_sure.toDouble / (winners + losers + not_sure))
            )
